...
class Warehouse:
            ...
            for _ in range(shelves_to_place):
                ...
                while not placed and attempts < 50:  # Limit attempts
                    ...
                    if self.grid[y][x] == CellType.AISLE and self._is_valid_shelf_position(x, y):
                        ...
                        self.shelf_products[(x, y)] = {
                            'type': product,
                            'quantity': random.randint(3, 6)  # More items for required products
                        }
    ...
    def _draw_delivery_zones(self, screen):
        ...
        if delivery_sprite:
            for (col, row) in self.delivery_zones:
                ...
                scaled_delivery = pygame.transform.scale(delivery_sprite,
                                                       (self.cell_size, self.cell_size))
                screen.blit(scaled_delivery, (x, y))
                
                # Draw "DELIVERY" text on top
            ...
            # Fallback: draw simple delivery zones
            for (col, row) in self.delivery_zones:
                x = self.x_offset + col * self.cell_size
                y = self.y_offset + row * self.cell_size
    ...
    def _draw_shelves(self, screen):
        """Draw shelves with their specific product sprites."""
        # Get product sprites
        product_sprites = {
        ...
        for (col, row), shelf_info in self.shelf_products.items():
            ...
            # Get the appropriate shelf sprite
            shelf_sprite_name = f"shelf_{product_type.value}"
            shelf_sprite = self.assets.get(shelf_sprite_name)
            
            if shelf_sprite:
                # Draw the shelf
                scaled_shelf = pygame.transform.scale(shelf_sprite,
                                                    (self.cell_size, self.cell_size))
                screen.blit(scaled_shelf, (x, y))
                
                # Draw product item on shelf (if any quantity)
                if quantity > 0 and product_type in product_sprites:
                    product_sprite = product_sprites[product_type]
                    if product_sprite:
                        # INCREASE SIZE: from 50% to 70% of cell size
                        item_size = int(self.cell_size * 0.7)  # Changed from 0.5 to 0.7
                        scaled_product = pygame.transform.scale(product_sprite,
                                                              (item_size, item_size))
                        # Center product on shelf
                        item_x = x + (self.cell_size - item_size) // 2
                        item_y = y + (self.cell_size - item_size) // 2
                        screen.blit(scaled_product, (item_x, item_y))
                        
                        # Draw quantity indicator
                        if quantity > 1:
                            font = pygame.font.Font(None, 20)
                            text = font.render(str(quantity), True, (255, 255, 255))
                            text_rect = text.get_rect(bottomright=(x + self.cell_size - 5, 
                                                                  y + self.cell_size - 5))
                            screen.blit(text, text_rect)
            else:
                # Fallback: draw colored rectangle with product info
                color = (150, 100, 50) if quantity > 0 else (100, 70, 30)
                pygame.draw.rect(screen, color, (x, y, self.cell_size, self.cell_size))
                pygame.draw.rect(screen, (120, 80, 40), (x, y, self.cell_size, self.cell_size), 2)
                
                # Draw product initial
                font = pygame.font.Font(None, 24)
                text = font.render(product_type.value[0].upper(), True, (255, 255, 255))
                text_rect = text.get_rect(center=(x + self.cell_size//2, y + self.cell_size//2))
                screen.blit(text, text_rect)
                ...
                if quantity > 0:
                    font = pygame.font.Font(None, 20)
                    text = font.render(str(quantity), True, (255, 255, 255))
                    text_rect = text.get_rect(bottomright=(x + self.cell_size - 5, y + self.cell_size - 5))
                    screen.blit(text, text_rect)